<template>
  <div>
    <div class="charts-cut">
      <span @click="chartCut('table')"
        :class="{current:groupItem ==='table'}">
        <svg-icon icon-class="tableCharts" /></span>
      <span @click="chartCut('piechart')"
        :class="{current:groupItem ==='piechart'}">
        <svg-icon icon-class="pieCharts" /></span>
    </div>
    <div v-show="groupItem === 'table'">
      <!-- 表格展示模块 -->
      <egrid v-loading.body="listLoading"
        element-loading-text="Loading"
        border
        fit
        highlight-current-row
        :column-key-map="{ label: 'name' }"
        :column-type="['index']"
        :column-type-props="columnTypeProps"
        :data="list"
        :columns="columns">
      </egrid>
      <!-- 表格分页 -->
      <el-pagination v-if="total !== 0"
        class="tiny-pagination"
        @current-change="handleCurrentChange"
        @size-change="handleSizeChange"
        background
        layout="total, prev, sizes, pager, next, jumper"
        :total="total"
        :page-size="pageSize"
        :page-sizes="[10, 20, 40]"
        :current-page="currentPage">
      </el-pagination>
    </div>
    <div v-if="groupItem === 'piechart'"
      class="pie">
      <div class="pieModel">
        <pie-model :date="date"></pie-model>
      </div>
      <div class="pieVersion">
        <pie-version :date="date"></pie-version>
      </div>
    </div>
  </div>
</template>
<script>
// import pieChart from './_components/pieChart';
import pieModel from './_components/pieModel';
import pieVersion from './_components/pieVersion';
import userApi from '@/api/user';
import mixin from '@/utils/mixin';
export default {
  name: 'equType',
  mixins: [mixin],
  props: ['date', 'name'],
  components: {
    // pieChart,
    pieModel,
    pieVersion
  },
  data() {
    return {
      groupItem: 'table',
      list: [],
      total: 0,
      listLoading: false,
      currentPage: 1, // 当前页码
      pageSize: 10 // 页面大小
    };
  },
  computed: {
    columnTypeProps() {
      return {
        index: {
          fixed: 'left',
          label: this.$t('views.index'),
          width: 50,
          align: 'center'
        }
      };
    },
    columns() {
      return [
        // 表头数据
        { name: this.$t('views.deviceModel'), prop: 'devModel' },
        { name: this.$t('views.devVersion'), prop: 'devVersion' },
        { name: this.$t('views.num'), prop: 'num' },
        { name: this.$t('views.statisticDate'), prop: 'dateType' }
      ];
    }
  },
  watch: {
    date() {
      this.init();
    },
    name() {
      this.init();
    }
  },
  created() {
    this.init();
  },
  methods: {
    chartCut(name) {
      this.groupItem = name;
    },
    handleCurrentChange(val) {
      this.currentPage = val;
      this.init();
    },
    handleSizeChange(val) {
      this.pageSize = val;
      this.init();
    },
    init() {
      this.listLoading = true;
      // 查询数据库获取表格信息
      const data = {
        startDate: this.startDate,
        endDate: this.endDate,
        pageNo: (this.currentPage - 1) * this.pageSize,
        pageSize: this.pageSize,
        devModel: this.name === null ? '未知' : this.name
      };
      userApi
        .devVersion(data)
        .then(res => {
          if (res.robj.items.length > 0) {
            this.total = res.robj.total;
            this.list = res.robj.items.map(item => {
              if (item.devModel === null || item.devModel === undefined) {
                item.devModel = '未知';
              }
              if (item.devVersion === null || item.devVersion === undefined) {
                item.devVersion = '未知';
              }
              return item;
            });
            this.listLoading = false;
          } else {
            this.listLoading = false;
            this.total = 0;
            this.list = [];
          }
        })
        .catch(res => {
          this.$message.error('数据请求失败!');
        });
    }
  }
};
</script>

<style lang="scss">
.pie {
  width: 100%;
  height: 400px;
}
.pieModel {
  width: 49%;
  float: left;
}
.pieVersion {
  width: 49%;
  float: right;
}
</style>
